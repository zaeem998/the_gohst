...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
  send_large_msg(receiver, "😡لتلعَب بكَيفك فقَطَ أَلمطور يحَق لهَ😡")
  end
  for k,v in pairs(result) do
    ...
    moderators = {},
      ...
      save_data(_config.moderation.data, data)
    local text = '✅ تمً تًفعَـيل ألمَجمَوَعـه ✅.'
...
local title ="👥 معلومات عن مجموعة🌐: ["..result.title.."]\n\n"
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return '👮 تم ✅ فتح الروابط في المجموعه يمكنك الارسال الان 🔓'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '🖕🏿 اللغه العربيه بالفعل مفتوحه 🔓✅'
  else
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return '🖕🏿 التحذير بالفعل مفتوح 🔓✅'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔧 تم فتح التحذير 🔓✅'
  end
end
...
local function unset_public_membermod(msg, data, target)
  ...
  data[tostring(target)]['long_id'] = msg.to.long_id 
    save_data(_config.moderation.data, data)
    return '🚫 المجموعه الان ليست عامه 🚫'
  end
end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, '❌🔧 الَمَجموَعهْ ليستَ فعأَلهَ ❌.')
  end
  if data[group]['moderators'][tostring(user_id)] then
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return '❌🔧 الَمَجموَعهْ ليستَ فعأَلهَ ❌.'
  end
  — determine if table is empty
  if next(data[tostring(msg.to.id)]['moderators']) == nil then
    return '❌ لآَ يِـوَجـدَ أَدمنـيِهَ حـأليـأَ ❌.'
  end
  local i = 1
  local message = '\n😎 قأئمَهَ ألآَدمــنيِهَ 😎 ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
...
function get_message_callback(extra, success, result)
  ...
  elseif get_cmd == 'ايدي' and result.action then
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
         return send_large_msg("channel#id"..channel_id, "😪 لاتمسلت بكيفك لايمكنك طرد الادمن أو المديرَ🖕🏿")
    end
...
local function run(msg, matches)
  ...
  elseif msg.to.type == 'channel'then
    if matches[1] == 'ترقيه سوبر' then
      ...
      return "المجموعة ⚡️ خارقة  🛡 بالفعل ✅"
    end
  end
  ...
  local data = load_data(_config.moderation.data)
    if matches[1] == 'تفعيل المجموعه' and not matches[2] then
      ...
      if is_super_group(msg) then
        return reply_msg(msg.id, '✅ ألمَجمَوَعــهَ بألــتأكيَدَ مفعَلهَ ✅..', ok_cb, false)
      end
    ...
    if matches[1] == 'تعطيل المجموعه' and is_admin1(msg) and not matches[2] then
      if not is_super_group(msg) then
        return reply_msg(msg.id, '✅ ألمَجمَوَعــهَ بألــتأكيَدَ تَمَ تَعَطيَلهَأَ ✅..', ok_cb, false)
      end
    ...
    if matches[1] == "الاداريين" then
      ...
      member_type = '🛡 قأئـمَهَ أَلأدرييــنَ 🛡'
...
if matches[1] == 'تغير الرابط' and is_momod(msg)then
      ...
      local receiver = get_receiver(msg)
        if success == 0 then
          ...
          send_large_msg(receiver, "تم✅ تغير الرابط 👥 ")
    ...
    if matches[1] == 'الرابط' then
      ...
      if not group_link then
        return "✅يرجئ ارسال [/تغير الرابط] لانشاء رابط المجموعه✅"
      end
...
if matches[1] == 'تنزيل ادمن' then
      ...
      if not is_owner(msg) then
        return "🖕🏿لتلعَب بكَيفك فقَطَ المدير او الاداري يحَق لهَ✅"
      end
      if type(msg.reply_id) ~= "nil" then
        local cbreply_extra = {
          get_cmd = 'تنزيل ادمن',
...
if matches[1] == 'مسح' then
      ...
      if not is_momod(msg) then
        return "🖕🏿لتلعَب بكَيفك فقَطَ المدير  يحَق لهَ✅"
      end
      if matches[2] == 'الادمنيه' then
        if next(data[tostring(msg.to.id)]['moderators']) == nil then
          return '😎 عذرا لا يوجد ادمنيه في المجموعه ليتم مسحهم 😎'
        end
        for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
      ...
      if matches[2] == 'القوانين' then
        ...
        if data[tostring(msg.to.id)][data_cat] == nil then
          return "🖕🏿عذرا لا يوجد قوانين في المجموعه ليتم مسحها ✅"
        end
      ...
      if matches[2] == 'الوصف' then
        ...
        local data_cat = 'description'
        if data[tostring(msg.to.id)][data_cat] == nil then
          return '🖕🏿عذرا لا يوجد وصف في المجموعه ليتم مسحه ✅'
        end
        data[tostring(msg.to.id)][data_cat] = nil
        save_data(_config.moderation.data, data)
      ...
      if matches[2] == 'المكتومين' then
        ...
        local hash =  'mute_user:'..chat_id
          redis:del(hash)
        return "🖕🏿 تم مسح قائمه المكتومين ✅"
      end
      if matches[2] == 'المعرف' and is_admin1(msg) then
        local function ok_username_cb (extra, success, result)
          local receiver = extra.receiver
          if success == 1 then
            send_large_msg(receiver, "😞 تم مسح معرف المجموعه  ✅")
          elseif success == 0 then
            send_large_msg(receiver, "😕غذرا فشل مسح معرف المجموعه ✅")
          end
        end
        local username = ""
        channel_set_username(receiver, username, ok_username_cb, {receiver=receiver})
